...
@Component({
  selector: 'app-pmsrs',
  templateUrl: './pmsrs.component.html',
  styleUrls: ['./pmsrs.component.css']
})
export class PmsrsComponent implements OnInit {
...
acknowledge()
  {
    ...
    this.https.post<SRSDetails>('http://localhost:8080/project/acknowledgeSRS', this.currentsrs).subscribe(
      response => {
        this.currentsrs = response;
        console.log(this.currentsrs);
        alert('SRS was acknowledged sucessfully');
  ...
  saveComments()
  {
    var encrypted = this.EncrDecr.set('123456$#@$^@1ERF', 'empid');
    this.currentsrs.employeeid = this.EncrDecr.get('123456$#@$^@1ERF', this.cookieService.get(encrypted));
    this.https.post<SRSDetails>('http://localhost:8080/project/savePMComment1', this.currentsrs1).subscribe(
      response => {
        this.currentsrs1 = response;
        console.log(this.currentsrs1);
        this.currentsrs.employeeid = this.EncrDecr.get('123456$#@$^@1ERF', this.cookieService.get(encrypted));
    this.https.post<SRSDetails>('http://localhost:8080/project/savePMComment2', this.currentsrs2).subscribe(
      response => {
        this.currentsrs2 = response;
        console.log(this.currentsrs2);
        this.getProjectDetails();
  ...
  mail()
  {
    var encrypted = this.EncrDecr.set('123456$#@$^@1ERF', 'projectid');
    this.currentproject.projectid = this.EncrDecr.get('123456$#@$^@1ERF', this.cookieService.get(encrypted));
    this.https.post<Project>('http://localhost:8080/project/manualmail', this.currentproject).subscribe(
      response => {
        this.currentproject = response;
        console.log(this.currentproject);
        alert('SRS sending was successful');
      });
